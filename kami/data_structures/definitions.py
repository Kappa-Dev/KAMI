    ...
    def generate_graph(self, reference_graph, gene_node):
        ...
        for region in self.removed_components["regions"]:
            ...
            if region_node:
                ...
                warnings.warn(
                    "Region '{}' of the protoform node '{}' was not identified in the reference graph!".format(
                        region, gene_node))
        for site in self.removed_components["sites"]:
            ...
            if site_node:
                ...
                warnings.warn(
                    "Site '{}' of the protoform node '{}' was not identified in the reference graph!".format(
                        site, gene_node))
        for residue in self.removed_components["residues"]:
            ...
            if residue_node:
                ...
                warnings.warn(
                    "Residue '{}' of the protoform node '{}' was not identified in the reference graph!".format(
                        residue, gene_node))
        for state in self.removed_components["states"]:
            ...
            if state_node:
                ...
                warnings.warn(
                    "State '{}' of the protoform node '{}' was not identified in the reference graph!".format(
                        state, gene_node))
...
class Definition:
    ...
    Attributes
    ----------
    protoform : kami.data_structures.entities.Protoform
        Reference protoform object
    products : dict
    ...
    def generate_rule(self, reference_graph, meta_typing):
        ...
        for i, product in enumerate(self.products):
            ...
            for n in product_graph.nodes():
                ...
                if protoform_graph.meta_typing[n] == "protoform":
    ...
    def from_json(cls, json_dict):
        ...
        return cls(Protoform(json_dict["protoform"]), products)
...
#             # Create a Protoform object for every product
...
#         protoform_gene = keys_by_value(protoform_graph.meta_typing, "protoform")[0]
...
#             protoform_node = node
#             product_nodes = []
#             for product, protoform in product_genes.items():
