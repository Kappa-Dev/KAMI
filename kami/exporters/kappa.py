...
from kami.data_structures.entities import (State, Residue, Site,
                                           Region, Protein, RegionActor,
                                           SiteActor)
...
def _generate_agent_states(identifier, ag_typing, agent, ag_uniprot_id, agents,
    ...
    for s in states:
        if s not in to_ignore:
            ...
            state_attrs = identifier.graph.get_node(s)
        ...
        for r in regions:
            if r in template_rel[role + "_partner_region"]:
                ...
                agent_bnd_sites.append(agents[ag_uniprot_id][
                    "region_bnd_sites"][(ag_region, ag_bnd)][0])
    else:
        agent_bnd_sites.append(agents[ag_uniprot_id][
...
def _generate_initial_conditions(isoforms, identifier, agents,
                                 initial_conditions, default_concentation):

    def _retreive_states_and_bounds(comp, n):
        # ref_component = identifier.identify_component(
        #     component, protein_node)
        if isinstance(comp, State):
            for s_node, data in protein_states.items():
                if comp.name == data[0]:
                    state_nodes[s_node] = (1 if comp.test else 0, n)
        elif isinstance(comp, Residue):
            _retreive_states_and_bounds(comp.state, n)
        elif isinstance(comp, Site):
            for residue in comp.residues:
                _retreive_states_and_bounds(residue, n)
            for state in comp.states:
                _retreive_states_and_bounds(state, n)
            for bound in comp.bound_to:
                _retreive_bounds(comp, bound, n)
        elif isinstance(comp, Region):
            for site in comp.sites:
                _retreive_states_and_bounds(site, n)
            for residue in comp.residues:
                _retreive_states_and_bounds(residue, n)
            for state in comp.states:
                _retreive_states_and_bounds(state, n)
    ...
    def _retreive_bounds(left_comp, right_comp, n):
        # if isinstance(right_comp, Protein):
        #     pass
        # elif isinstance(right_comp, RegionActor):
        #     pass
        # elif isinstance(right_comp, SiteActor):
        #     pass
        pass

    print(isoforms)
    for agent_uniprotid, data in agents.items():
        print("Agent UP: ", agent_uniprotid)
        print("\t Name: ", data["agent_name"])
        print("\t Variants: ", data["variants"])
        print("\t Kami sites: ", data["kami_sites"])
        print("\t Direct bnd sites: ", data["direct_bnd_sites"])
        print("\t Region bnd sites: ", data["region_bnd_sites"])

    condition_strs = []
    if len(initial_conditions) > 0:
        for condition in initial_conditions:
            protoform_uniprotid = condition.canonical_protein.protoform.uniprotid
            protein_node = identifier.identify_protein(
            ...
            if condition.canonical_protein.name:
                ...
                if variant_name in agents[
                        protoform_uniprotid]["variants"].values():
                    variant_str = "variant{{{}}}".format(variant_name)
                else:
                    warnings.warn(
                        "Variant '{}' is not present in the signature of '{}', ".format(
                            variant_name, agents[
                                protoform_uniprotid]["agent_name"]) +
                        "generating initial condition for a default agent",
                        KappaGenerationWarning)

            condition_strs.append(
                "%init: {} {}({})".format(
                    condition.canonical_count,
                    agents[protoform_uniprotid]["agent_name"],
                    variant_str)
            )

            if len(condition.stateful_components) > 0 or\
                    len(condition.bounds) > 0:
                protein_states = {}
                for s in identifier.get_attached_states(protein_node):
                    # get state value (True always renders to 1, False to 0)
                    state_attrs = identifier.graph.get_node(s)
                    protein_states[s] = (
                        list(state_attrs["name"])[0],
                        list(state_attrs["test"])
                    )

                # Add stateful and bound to initial conditions
                state_nodes = dict()
                bound_to = dict()
                for component, count in condition.stateful_components:
                    _retreive_states_and_bounds(component, count)

                # Add bound initial conditions
                for component, count in condition.bounds:
                    # direct_bnds = [
                    #     bnd
                    #     for bnd in identifier.get_attached_bnd(protein_node)
                    #     if identifier.graph.exists_edge(protein_node, bnd)
                    # ]
                    if isinstance(component, Protein):
                        partner_id = identifier.identify_protein(component)
                        for bnd_node_id, site in agents[
                                protoform_uniprotid]["direct_bnd_sites"].items():
                            if identifier.graph.exists_edge(partner_id, bnd_node_id):
                                print("YEEEY")
                                bound_to

                    _retreive_bounds(
                        condition.canonical_protein, component, count)

                for state_node, (value, count) in state_nodes.items():
                    site_name = agents[
                        protoform_uniprotid]["stateful_sites"][state_node]

                    state_str = (
                        variant_str + "," if len(variant_str) > 0 else "" +
                        "{}{{{}}}".format(site_name, value)
                    )
                ...
                for agent_uniprotid, (my_site, other_site, count) in bound_to.items():
                    agent_name = agents[agent_uniprotid]["agent_name"]
                    bound_str = (
                        variant_str + "," if len(variant_str) > 0 else "" +
                        "{}[{}.{}]".format(
                            my_site, other_site, agent_name)
                    )
                    condition_strs.append(
                        "%init: {} {}({})".format(
                            count,
        ...
        for agent, agent_data in agents.items():
            ...
            if len(agent_data["variants"]) > 1:
                for variant_name in agent_data["variants"].values():
                    variant_str = "variant{{{}}}".format(variant_name)
                    condition_strs.append(
                        "%init: {} {}({})".format(
                            default_concentation,
...
def generate_kappa(model, initial_conditions=None, default_concentation=100):
    ...
    for isoform, (proteins, hgnc) in isoforms.items():
        ...
        for node, name in proteins.items():
            if name is not None:
                variants[node] = _normalize_variant_name(name)
            else:
        ...
        for protein, variant_name in variants.items():
            ...
            # Find kami sites (a Kappa site per every Kami site)
            ...
            # Find direct bnd sites (a Kappa site per every direct bnd)
            ...
            for bnd in direct_bnds:
                ...
                partner_nodes = [
                    b
                    for b in identifier.graph.predecessors(bnd)
                    if b != protein
                ]
                agents[isoform]["direct_bnd_sites"][bnd] = (
                    bnd_name, partner_nodes)

            # Find region bnd sites (a Kappa site per bnd through a region)
            ...
            for r in regions:
                ...
                for bnd in identifier.successors_of_type(r, "bnd"):
                    ...
                    agents[isoform]["region_bnd_sites"][(r, bnd)] = (
                        bnd_name,
                        partner_nodes
                    )
            ...
            if "substrate" in template_rel.keys():
                ...
                for substrate in substrates:
                    ...
                    substrate_name, substrate_variant_state = _generate_agent_name(
                        identifier, ag_typing, substrate,
    ...
    init = _generate_initial_conditions(
        isoforms, identifier, agents, initial_conditions, default_concentation)
    return header + signatures + rule_repr + variables + "\n".join(init)


def generate_kappa_from_corpus(corpus, definitions, concentations=None,
